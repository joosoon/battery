...
if __name__ == '__main__':
    ...
    for i, cell_id in enumerate(tqdm(cell_ids)):
        ...
        for data_type, crates in gts_real.items():
            ...
            if args.vis_3d:
                ...
                # df['cp'] = ys.tolist()
                # df['ap'] = zs.tolist()
                # df['label'] = ['taret' for _ in range(len(xs))]
                # df = pd.DataFrame(df)
                # fig = px.scatter_3d(
                #             df, 
                #             x='cl', 
                ...
                # df['cp'] = ys.tolist()
                # df['ap'] = zs.tolist()
                # df['label'] = ['result' for _ in range(len(xs))]
                # df = pd.DataFrame(df)
                # fig.add_trace(
                #         go.Scatter3d(
                #             x=xs, 
                ...
                # fig.add_trace(
                #         go.Scatter3d(
                #             x=[query_inputs['cathode_loading_density'].item()], 
                #             y=[query_inputs['cathode_porosity'].item()], 
                #             z=[query_inputs['anode_porosity'].item()],
                #             mode='markers',
                #             name='GT',
                #             opacity=1
                #             ))
                ...
                # save_name = os.path.join(save_dir, '{}_{}_3d.html'.format(data_type, cell_id))
                ...
                # keys = ['cathode_loading_density', 'cathode_porosity', 'anode_porosity']
                # for key_i, key in enumerate(keys):
                #     plt.subplot(2, 3, key_i+1)  
                #     plt.hist(ins_syn[key], bins=30)
                #     plt.title(key)
                
                #     plt.subplot(2, 3, key_i+4)  
                #     plt.hist(result_inputs[key], bins=30)
                #     plt.title(key)
                ...
                save_dir = os.path.join('results_syn', args.csv_name, 
                                        ...
                                        # 'vis', 'search')
                os.makedirs(save_dir, exist_ok=True)
                save_name = os.path.join(save_dir, '{}_{}_3d_tol_{}.png'.format(data_type, cell_id, tolerance))
                plt.savefig(save_name, dpi=500)
                plt.clf()
    ...
    print("ACCURACY: {:.2f}% ({}/{})".format(acc_all, sum(records), len(records)))
    ...
    print("---" * 20)
    print("AVG SELECT RATIO")
    print("...     ALL: {:.2f}%".format(avg_select_ratio_all))
    print("... SUCCESS: {:.2f}%".format(avg_select_ratio_suc))
    ...
    print(avg_select_ratio_suc, end=',')
    print()
